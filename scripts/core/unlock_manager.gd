...
func add_unlocks_for_victory(result: Dictionary, unlocked_messages: Array[String]) -> void:
	...
	match String(result.get("map_id", "")):
		...
		"map3":
			...
			try_unlock("tank", "prism_sentinel", "Prism Sentinel tank", unlocked_messages)
		"map4":
		...
		"map13":
			...
			try_unlock("tank", "reef_savant", "Reef Savant tank", unlocked_messages)
		"map14":
			try_unlock("map", "map15", "Umbral Vault map", unlocked_messages)
...
func get_next_unlock_goal_lines() -> Array[String]:
	...
	elif not is_map_unlocked("map15"):
		lines.append("- Map: Win Solar Bastion at 30:00 to unlock Umbral Vault.")
